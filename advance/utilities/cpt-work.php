    ...
    function apollo13framework_works_nav() {
        ...
        if($navigate_through_categories){
            ...
            if($count_terms > 0){
                ...
                $title = sprintf(esc_html__( 'Back to %s', 'rife' ), $term->name);
	        ...
	        /* translators: %s: page title */
            $title = sprintf(esc_html__( 'Back to %s', 'rife' ), get_the_title( $works_id ));
            if($works_id !== '0'){
        ...
        if( $navigate_through_categories ) {
	        next_post_link( '%link', '<span class="fa fa-long-arrow-'.( is_rtl() ? 'right' : 'left' ).'" title="%title - %date"></span> ' . esc_html__( 'Previous', 'rife' ), true, '', A13FRAMEWORK_CPT_WORK_TAXONOMY );
        }
        else {
	        next_post_link( '%link', '<span class="fa fa-long-arrow-'.( is_rtl() ? 'right' : 'left' ).'" title="%title - %date"></span> ' . esc_html__( 'Previous', 'rife' ) );
        }

        echo $show_back_btn? '<a href="'.esc_url($href).'" title="'.esc_attr($title).'" class="to-cpt-list fa fa-th"></a>' : '';

	    if( $navigate_through_categories ) {
            previous_post_link( '%link', esc_html__( 'Next', 'rife' ) . ' <span class="fa fa-long-arrow-'.( is_rtl() ? 'left' : 'right' ).'" title="%title - %date"></span>', true, '', A13FRAMEWORK_CPT_WORK_TAXONOMY );
        }
        else{
            previous_post_link( '%link', esc_html__( 'Next', 'rife' ) . ' <span class="fa fa-long-arrow-'.( is_rtl() ? 'left' : 'right' ).'" title="%title - %date"></span>' );
        }
		...
		if ( ! $query->have_posts() ):
			...
			<div class="formatter">
				<div class="real-content empty-blog">
					<?php
					echo '<p>'.esc_html__( 'Apologies, but no results were found for the requested archive.', 'rife' ).'</p>';
		...
		for ( $post_number = 0; $post_number < $number_of_posts; $post_number ++ ) {
			...
			if ( post_password_required( $post_id ) ) {
				...
				if( $apollo13framework_a13->get_option( 'works_list_protected_titles', 'on' ) === 'on' ){
					$html .= '<span class="fa fa-lock"></span>' . esc_html__( 'This content is password protected', 'rife' );
				}
				else{
				...
				$html .= '<p>' . esc_html__( 'Click and enter your password to view content', 'rife' ) . '</p>';
				$html .= '</div>';
				$html .= '</div>';
        ...
        if ( count($__search) ) {
            ...
            if ($r->have_posts()) :
                echo '<div class="similar-works">
                        	<h3>'.esc_html__('Similar Works', 'rife' ).'</h3>
