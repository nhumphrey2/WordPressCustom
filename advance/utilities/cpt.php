    ...
    function apollo13framework_cpt_meta_fields(){
        ...
        for($i = 0; $i <= $custom_fields_number; $i++){
	        if( $i === 0 ) {
		        ...
		        if ( strlen( $temp ) ) {
			        $temp = esc_html__( 'Website', 'rife' ).':'.$temp;
		        }
	        }
	        else{
		        //custom fields
...
if(!function_exists('apollo13framework_loop_pagination')){
	...
	function apollo13framework_loop_pagination($args){
		$args['type'] = 'plain';
		$args['prev_text'] = _x( 'Previous', 'previous product', 'rife' );
		$args['next_text'] = _x( 'Next', 'next product', 'rife' );
...
function apollo13framework_print_media_filters($id = null){
	...
	if ( count( $filters ) ){
		...
		echo '<li class="selected" data-filter="__all"><a href="' . esc_url( apollo13framework_current_url() ) . '">' . esc_html__( 'All', 'rife' ) . '</a></li>';
...
function apollo13framework_print_proofing_filters($id = null){
	...
	<ul class="category-filter single-album-proofing-filter clearfix">
		<li class="selected" data-filter="__all"><a><?php esc_html_e( 'All', 'rife' ); ?></a></li>
		<li data-filter="1"><a><?php esc_html_e( 'Selected', 'rife' ); ?></a></li>
		<li data-filter="0"><a><?php esc_html_e( 'Not selected', 'rife' ); ?></a></li>
		<li data-filter="2"><a><?php esc_html_e( 'Commented', 'rife' ); ?></a></li>
	</ul>
	<div id="done-with-proofing" <?php echo $hide_button? ' class="idle"' : ''; ?>>
		<div class="not-done" title="<?php esc_attr_e( 'When you are done with commenting and selecting photos/videos, click this button.', 'rife' ); ?>"><?php esc_html_e( 'I am done with Selecting Items', 'rife' ); ?></div>
		<div class="loading"><?php esc_html_e( 'Working ...', 'rife' ); ?></div>
		<div class="done"><?php esc_html_e( 'Accepted Items:', 'rife' ); ?> <span class="counter"></span> <i class="fa fa-check"></i></div>
	</div>
	<?php
}
