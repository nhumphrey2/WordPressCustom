...
if(!function_exists('apollo13framework_albums_nav')){
    ...
    function apollo13framework_albums_nav() {
        ...
        if($navigate_through_categories){
	        ...
	        /* translators: %s: page title */
            $title = sprintf(esc_html__( 'Back to %s', 'rife' ), get_the_title( $albums_id ));
            if($albums_id !== '0'){
...
if(!function_exists('apollo13framework_albums_outside_nav')){
    ...
    function apollo13framework_albums_outside_nav() {
        ...
        if( $navigate_through_categories ) {
	        next_post_link( '%link', '<span class="fa fa-long-arrow-'.( is_rtl() ? 'right' : 'left' ).'" title="%title - %date"></span> ' . esc_html__( 'Previous', 'rife' ), true, '', A13FRAMEWORK_CPT_ALBUM_TAXONOMY );
        }
        else {
	    ...
	    if( $navigate_through_categories ) {
            previous_post_link( '%link', esc_html__( 'Next', 'rife' ) . ' <span class="fa fa-long-arrow-'.( is_rtl() ? 'left' : 'right' ).'" title="%title - %date"></span>', true, '', A13FRAMEWORK_CPT_ALBUM_TAXONOMY );
        }
        else{
