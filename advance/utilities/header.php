...
if ( ! function_exists( 'apollo13framework_header_logo' ) ) {
	...
	function apollo13framework_header_logo() {
		...
		$logos_name_map = array(
			...
			'dark'   => 'header_dark_logo_image',
			'light'  => 'header_light_logo_image'
		);
		//fill sources & sizes
		...
		$svg_logo            = $img_logo && $apollo13framework_a13->get_option( 'logo_svg', 'off' ) === 'on';
		...
		if ( $img_logo ) {
			if( $svg_logo && strlen( $logo_sizes['normal']['id'] ) && get_post_mime_type( $logo_sizes['normal']['id'] ) === 'image/svg+xml' ){
				$svg = apollo13framework_read_contents( get_attached_file( $logo_sizes['normal']['id'] ) );
				$html .= '<span class="scaling-svg" style="padding-bottom:'.esc_attr(apollo13framework_get_svg_height_ratio($svg)).'%;">'.$svg.'</span>';
				$html .= '<meta itemprop="logo" content="' . esc_attr( $logo_sources['normal'] ) . '"/>';

			}
			else{
				$image_alt = get_post_meta( $logo_sizes['normal']['id'], '_wp_attachment_image_alt', true);
				$html .= '<img src="' . esc_url( $logo_sources['normal'] ) . '" alt="' . esc_attr( $image_alt ) . '" width="' . esc_attr( $logo_sizes['normal']['width'] ) . '" height="' . esc_attr( $logo_sizes['normal']['height'] ) . '"'.apollo13framework_get_schema_args('logo_image').' />';
			}
		}
		else {
		...
		if ( $img_logo && $is_horizontal && $logo_from_variants ) {
			foreach($logo_sources as $variant => $src){
				...
				if(strlen($src)){
					...
					if( $svg_logo && strlen( $logo_sizes[$variant]['id'] ) && get_post_mime_type( $logo_sizes[$variant]['id'] ) === 'image/svg+xml' ){
						$svg = apollo13framework_read_contents( get_attached_file( $logo_sizes[$variant]['id'] ) );
						$html .= '<span class="scaling-svg" style="padding-bottom:'.esc_attr(apollo13framework_get_svg_height_ratio($svg)).'%;">'.$svg.'</span>';
					}
					else{
						$image_alt = get_post_meta( $logo_sizes[$variant]['id'], '_wp_attachment_image_alt', true);
						$html .= '<img src="' . esc_url( $src ) . '" alt="' . esc_attr( $image_alt ) . '" width="' . esc_attr( $logo_sizes[ $variant ]['width'] ) . '" height="' . esc_attr( $logo_sizes[ $variant ]['height'] ) . '" />';
					}
					$html .= '</a>';
				}
			}
...
if ( ! function_exists( 'apollo13framework_header_menu' ) ) {
	...
	function apollo13framework_header_menu( $walker = '' ) {
		...
		if ( $walker === 'one-line-left' ) {
			$menu_args['menu_class'] .= ' left-part';
			$no_menu_args['menu_class'] .= ' left-part';

			if($is_menu){
				/** @noinspection PhpIncludeInspection */
				require_once( get_theme_file_path( 'advance/walkers/left-right.php' ) );
				$menu_args['walker'] = new A13FRAMEWORK_menu_one_line_left_walker;
			}
			else{
				/** @noinspection PhpIncludeInspection */
				require_once( get_theme_file_path( 'advance/walkers/left-right-no-menu.php' ) );
				$no_menu_args['walker'] = new A13FRAMEWORK_page_tree_one_line_left_walker;
			}
		}
		elseif ( $walker === 'one-line-right' ) {
			$menu_args['menu_class'] .= ' right-part';
			$no_menu_args['menu_class'] .= ' right-part';

			if($is_menu){
				/** @noinspection PhpIncludeInspection */
				require_once( get_theme_file_path( 'advance/walkers/left-right.php' ) );
				$menu_args['walker'] = new A13FRAMEWORK_menu_one_line_right_walker;
			}
			else{
				/** @noinspection PhpIncludeInspection */
				require_once( get_theme_file_path( 'advance/walkers/left-right-no-menu.php' ) );
				$no_menu_args['walker'] = new A13FRAMEWORK_page_tree_one_line_right_walker;
			}
		}
		else {
			if($is_menu){
...
if ( ! function_exists( 'apollo13framework_get_header_toolbar' ) ) {
	...
	function apollo13framework_get_header_toolbar( &$icons ) {
		...
		//prepare icons HTML
		$tools_html =
		        ( $basket_sidebar ? '<button id="basket-menu-switch" class="'.esc_attr($basket_sidebar_icon).' tool" title="' . esc_attr__( 'Shop sidebar', 'rife' ) . '"><span id="basket-items-count" class="zero">' . esc_html( $woocommerce->cart->cart_contents_count ) . '</span><span class="screen-reader-text">' . esc_attr__( 'Shop sidebar', 'rife' ) . '</span></button>' : '' ) .
		        ( $header_search ? '<button id="search-button" class="'.esc_attr($header_search_icon).' tool" title="' . esc_attr_x( 'Search', 'action', 'rife' ) . '"><span class="screen-reader-text">' . esc_attr_x( 'Search', 'action', 'rife' ) . '</span></button>' : '' ) .
		        ( $hidden_sidebar ? '<button id="side-menu-switch" class="'.esc_attr($hidden_sidebar_icon).' tool" title="' . esc_attr__( 'More info', 'rife' ) . '">'.($hs_type === 'animated'? '<i></i>' : '').'<span class="screen-reader-text">' . esc_attr__( 'More info', 'rife' ) . '</span></button>' : '' ) .
		        ( $menu_overlay ? '<button id="menu-overlay-switch" class="'.esc_attr($menu_overlay_icon).' tool" title="' . esc_attr__( 'Main menu', 'rife' ) . '">'.($mo_type === 'animated'? '<i></i>' : '').'<span class="screen-reader-text">' . esc_attr__( 'Main menu', 'rife' ) . '</span></button>' : '' ) .
		        ( $allow_mobile_menu ? '<button id="mobile-menu-opener" class="'.esc_attr($mobile_menu_icon).' tool" title="' . esc_attr__( 'Main menu', 'rife' ) . '">'.($mm_type === 'animated'? '<i></i>' : '').'<span class="screen-reader-text">' . esc_attr__( 'Main menu', 'rife' ) . '</span></button>' : '' ) .
