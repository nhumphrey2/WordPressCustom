...
if(!function_exists( 'apollo13framework_get_post_meta_data' )){
    ...
    function apollo13framework_get_post_meta_data() {
        ...
        if(
            ...
            $return .= esc_html__( 'Updated: ', 'rife' ).apollo13framework_modified_on();
        }
...
if(!function_exists('apollo13framework_under_post_content')){
    function apollo13framework_under_post_content() {
        ...
        wp_link_pages( array(
                'before' => '<div id="page-links"><span class="page-links-title">'. esc_html__( 'Pages: ', 'rife' ).'</span>',
                'after'  => '</div>'
	        )
        );
            ...
            if($is_prev || $is_next){
                ...
                if($is_prev){
                    ...
                    echo '<a href="'.esc_url( get_permalink($id) ).'" class="item prev">'
                         .'<span><i class="fa fa-long-arrow-'.( is_rtl() ? 'right' : 'left' ).'"></i> '.esc_html__( 'Previous article', 'rife' ).'</span>'
                ...
                if($is_next){
                    ...
                    echo '<a href="'.esc_url( get_permalink($id) ).'" class="item next">'
                         .'<span>'.esc_html__( 'Next article', 'rife' ).' <i class="fa fa-long-arrow-'.( is_rtl() ? 'left' : 'right' ).'"></i></span>'
...
if(!function_exists('apollo13framework_new_excerpt_more')){
    ...
    function apollo13framework_new_excerpt_more($link) {
        ...
        //return read more
        return '&hellip;
 <p> <a class="more-link" href="'. esc_url( get_permalink($post->ID) ) . '">'.esc_html__( 'Read more', 'rife' ).'</a></p>';
    }
}
...
if(!function_exists('apollo13framework_has_excerpt_read_more')){
    ...
    function apollo13framework_has_excerpt_read_more($content) {
        ...
        if( $apollo13framework_a13->get_option( 'blog_read_more', 'on' ) === 'on' && has_excerpt() ){
            $content .= '<p> <a class="more-link" href="'. esc_url( get_permalink($post->ID) ) . '">'.esc_html__( 'Read more', 'rife' ).'</a></p>';
        }
    ...
    function apollo13framework_comments_navigation() {
        if ( get_comment_pages_count() > 1 && get_option( 'page_comments' ) ) :
            ...
            <nav class="navigation comment-navigation">
                <h2 class="screen-reader-text"><?php esc_html_e( 'Comment navigation', 'rife' ); ?></h2>
                <div class="nav-links">
                    <?php
                    if ( $prev_link = get_previous_comments_link( esc_html__( 'Older Comments', 'rife' ) ) ) :
                        printf( '<div class="nav-previous">%s</div>', wp_kses_post( $prev_link ) );
                    endif;

                    if ( $next_link = get_next_comments_link( esc_html__( 'Newer Comments', 'rife' ) ) ) :
                    ...
                    if(post_password_required()){
                        ...
                        <div class="formatter">
                            ...
                            <div class="real-content">
                                <p><?php esc_html_e( 'To view it please enter your password below', 'rife' ); ?></p>
