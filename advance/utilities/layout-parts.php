    ...
    $prelaoder_bg_image_fit
		...
		if ( $prelaoder_type !== 'none' ) {
			...
			if( $content !== false ){
				...
				$css .= '.no_filesystem_preloader_animation{color:#fff;}';
			}
		}
	}
...
if(!function_exists('apollo13framework_search_form')){
	...
	function apollo13framework_search_form(
        ...
        $field_search = '<input' .
            ' placeholder="' . esc_attr__('Search &hellip;', 'rife' ) . '" ' .
        ...
        $form = '
                <form class="search-form" role="search" method="get" action="' . esc_url( home_url( '/' ) ) . '" >
                    <fieldset class="semantic">
                        ' . $field_search . '
                        <input type="submit" id="searchsubmit' . esc_attr( $search_id ) . '" title="'. esc_attr_x( 'Search', 'action', 'rife' ) .'" value="'. esc_attr_x( 'Search', 'action', 'rife' ) .'" />
