    ...
    echo '<div class="notice notice-warning inline"><p>' .
         sprintf(
             wp_kses(
                 ...
                 __( 'Theme options for this page can be found in <a href="%1$s">Appearance -&gt; Customize -&gt; %2$s</a>.', 'rife' ),
                 array(
                     'a'      => array(
                         'href' => array()
                     )
...
function apollo13framework__no_meta_shop_page_notice() {
    echo '<div class="notice notice-warning inline"><p>' .
         sprintf(
             ...
             esc_html__( 'Shop(WooCommerce) settings', 'rife' )
         ) . '</p></div>';
}
...
function apollo13framework__no_meta_albums_page_notice() {
    echo '<div class="notice notice-warning inline"><p>' .
         sprintf(
             ...
             esc_html__( 'Albums settings', 'rife' )
         ) . '</p></div>';
}
...
function apollo13framework__no_meta_works_page_notice() {
    echo '<div class="notice notice-warning inline"><p>' .
         sprintf(
             ...
             esc_html__( 'Works settings', 'rife' )
         ) . '</p></div>';
}
    ...
    if(current_user_can( 'edit_posts' )){
        ...
        add_meta_box(
            'apollo13_theme_options_1',
             __( 'Album media - Add images/videos', 'rife' ),
            'apollo13framework_meta_main_opts',
            A13FRAMEWORK_CUSTOM_POST_TYPE_ALBUM,
            'normal',
            'high',
        ...
        add_meta_box(
            'apollo13_theme_options_1',
            __( 'Work media - Add images/videos', 'rife' ),
            'apollo13framework_meta_main_opts',
            A13FRAMEWORK_CUSTOM_POST_TYPE_WORK,
            'normal',
            'high',
            ...
            */
            ...
            elseif ( $meta['type'] === 'multi-upload' ) { ?>
                ...
                <div class="a13-mu-container">
                    ...
                    echo esc_attr( __( 'Select/Upload images and videos', 'rife' ) );
                    ?>"<?php echo ( isset( $meta['media_type'] ) && strlen( $meta['media_type'] ) ) ?
                        ' data-media-type="'.esc_attr($meta['media_type']) . '"' : ''; ?> />
                    <span class="button button-large add-link-media"><?php esc_html_e( 'Add Video from Youtube/Vimeo', 'rife' ); ?></span>
                    <label class="button button-large"><input type="checkbox" id="mu-prepend" value="1" /><?php esc_html_e( 'Add items at beginning of the list', 'rife' ); ?>
                    </label>
                    <input id="a13-multi-remove" class="button button-large" type="button" value="<?php echo esc_attr( __( 'Remove selected', 'rife' ) ); ?>" disabled="disabled" />
                    <?php

                    do_action('apollo13framework_a13_multi_upload_tools');

                    apollo13framework_input_help_tip( __( 'To mark more items in Media Library and in below list, you can use <code>Ctrl</code>(<code>Cmd</code>) or <code>Shift</code> key while selecting them with mouse.', 'rife' ));
                    ?>
