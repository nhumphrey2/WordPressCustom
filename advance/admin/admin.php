...
if(!function_exists('apollo13framework_admin_js_parameters')){
    function apollo13framework_admin_js_parameters(){
        ...
        $params['messages']['duplicate_nava_anchors'] = esc_html__( 'There are rows in content with identical "Navigation anchor title" parameter, which has to be unique for each item. Please fix it by editing row parameters. Duplicated titles are:', 'rife' );
        $params['messages']['confirm_delete_nava'] = esc_html__( 'Do You want to delete selected One Page Navigation Pointer?', 'rife' );
...
if(!function_exists('apollo13framework_admin_pages')){
    ...
    function apollo13framework_admin_pages() {
        ...
        $temp = sprintf( esc_html__( '%s Import &amp; Info', 'rife' ), A13FRAMEWORK_OPTIONS_NAME_PART);
...
function apollo13framework_admin_footer() {
    ...
    echo '<div id="a13-fa-icons"><input type="search" value="" placeholder="'.esc_attr__( 'Start typing to filter icons', 'rife' ).'" /> ';
