...
class Apollo13Framework
{
    ...
    function start()
    {
         ...
         */
        define('A13FRAMEWORK_TPL_SLUG', 'rife');//it is not always same as directory of theme
        define('A13FRAMEWORK_OPTIONS_NAME_PART', 'Rife Pro');
        define('A13FRAMEWORK_THEME_ID_NUMBER', '69');
        define('A13FRAMEWORK_OPTIONS_NAME', 'apollo13_option_rife');
        define('A13FRAMEWORK_CACHE', 'apollo13_rife_cache');
        define('A13FRAMEWORK_THEME_VERSION', '2.3.5');
        define('A13FRAMEWORK_THEME_VER', A13FRAMEWORK_THEME_VERSION ); //legacy - do not use
        define('A13FRAMEWORK_MIN_COMPANION_VERSION', '1.6.2');
...
//		define('A13FRAMEWORK_CUSTOM_POST_TYPE_WORK_SLUG', 'work'); //just to show it also exist - defined in real a bit lower
    ...
    function setup()
    {
        ...
        //LANGUAGE
        load_theme_textdomain( 'rife', get_theme_file_path( 'languages' ) );
        ...
        register_nav_menus(array(
            'header-menu' => __( 'Site Navigation', 'rife' ),
            'top-bar-menu' => __( 'Alternative short top bar menu', 'rife' ),
        ));
    }
...
//            'images_manager' //no parent options here
    ...
    function is_companion_plugin_ready($fail_message = false, $silent = false){
        ...
        if ( empty( $plugins[$plugin_file] ) ) {
            ...
            $label = __( 'Install', 'rife' ) . ' : ' . __( 'Apollo13 Framework Extensions', 'rife' );
        }
        ...
        elseif ( is_plugin_inactive( $plugin_path ) ){
            ...
            $label = __( 'Activate', 'rife' ) . ' : ' . __( 'Apollo13 Framework Extensions', 'rife' );
        }
        ...
        elseif( version_compare( $plugins[$plugin_file]['Version'], A13FRAMEWORK_MIN_COMPANION_VERSION, '<'  ) ){
            ...
            $label = __( 'Update', 'rife' ) . ' : ' . __( 'Apollo13 Framework Extensions', 'rife' );
        }

        if(!$ready && !$silent){
            $message = $fail_message ? $fail_message : __( 'This feature requires Apollo13 Framework Extensions plugin to be active and in the proper version.', 'rife' );
