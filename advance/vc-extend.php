...
function vc_gitem_template_attribute_a13_post_body( $value, $data ) {
	...
	if ( post_password_required( $post->ID ) ) {
		...
		$html .= '<h2 class="post-title"><a href="' . esc_url( get_the_permalink( $post->ID ) ) . '"><span class="fa fa-lock"></span>' . esc_html__( 'This content is password protected', 'rife' ) . '</a></h2>';
		$html .= '<div class="real-content">';
		$html .= '<p>' . esc_html__( 'To view it please enter your password below', 'rife' ) . '</p>';
...
function vc_a13_post_meta_1_render() {
	return '<div class="a13_post_meta_data">{{ post_data:post_date }} ' . esc_html__( 'in', 'rife' ) . ' {{ a13_post_categories }}</div>';
}