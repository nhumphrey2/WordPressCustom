        ...
        $form = '
                <form class="search-form" role="search" method="get" action="' . esc_url( home_url( '/' ) ) . '" >
                    <fieldset class="semantic">
                        ' . $field_search . '
                        <input type="submit" id="searchsubmit' . esc_attr( $search_id ) . '" title="'. esc_attr__( 'Search', 'rife-free' ) .'" value="'. esc_attr__( 'Search', 'rife-free' ) .'" />
...
if(!function_exists('apollo13framework_get_title_bar')){
	...
	function apollo13framework_get_title_bar( $called_position = 'outside', $title = '', $subtitle = '') {
			...
			if( $called_position === 'outside' ){
				...
				if($page_type['post']){
					...
					return '<div class="post-hatom-fix">
								...
								'.apollo13framework_post_meta_under_content() . apollo13framework_post_meta_above_content().'
							</div>';
		...
		//main title
		$html .= '<h1 class="page-title entry-title">'.$title.'</h1>';//sometimes we add html here, so don't escape!
