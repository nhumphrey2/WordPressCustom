...
if(!function_exists('apollo13framework_theme_scripts')){
	...
	function apollo13framework_theme_scripts(){
        ...
        //Animation library
        wp_register_script( 'tweenmax', get_theme_file_uri( 'js/TweenMax.min.js' ), array('jquery'), '2.0.2', true);
		$script_depends[] = 'tweenmax';

        //APOLLO Slider
        wp_register_script( 'apollo13framework-slider', get_theme_file_uri( 'js/a13-slider' . $suffix . '.js' ), array('jquery', 'tweenmax'), A13FRAMEWORK_THEME_VERSION, true);
	...
	function apollo13framework_theme_styles(){
        ...
        if( ! is_customize_preview() ){
            if( function_exists( 'a13fe_generate_user_css' ) ){
                ...
                $last_modified = file_exists( $user_css_file ) ? filemtime( $user_css_file ) : '0';
