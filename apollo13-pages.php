...
function apollo13framework_apollo13_info() {
	...
	if( apollo13framework_is_companion_plugin_ready( esc_html__( 'Some features(like Albums, Works, shortcodes, plugins support) of the Theme requires an additional plugin before you will be able to use it. ', 'rife-free' ) ) ){
		echo '<p>'.esc_html__( 'You can check out what is new in the changelog or just move on with your usual work.', 'rife-free' ).
		     ' <a href="http://www.apollo13.eu/themes_update/apollo13_framework_theme/index.html#change-log">'.esc_html__( 'View the changelog.', 'rife-free').'</a>'.
		     '</p>';
		...
		echo '<p>'.esc_html__( 'If you have an existing website, you can also try to import one of our designs to speed up your work. You will have to do it without demo data.', 'rife-free' ).
		     ' <a href="'.esc_url( $apollo13framework_a13->get_docs_link('importer-configuration') ).'">'.esc_html__( 'How to do it is explained in documentation here.', 'rife-free').'</a>'.
		     '</p>';
	}
...
function apollo13framework_apollo13_help() {
	...
	<p><?php echo
		esc_html__( 'Online documentation is always most up to date if it comes to explaining how to work with theme. It will come handy as first source when you trying to work out problematic topics.', 'rife-free' );
		?></p>

	<h3 class="center"><a href="https://apollo13themes.com/rife/help/#tutorials"><?php echo esc_html__( 'Video Tutorials', 'rife-free' ); ?></a></h3>
	<p><?php
		echo esc_html__( 'We have prepared some basics tutorials on how to work with theme.', 'rife-free' );
		?></p>
	<p><?php
		echo esc_html__( 'We are planing to do more, but if you have idea for subject that we should cover in next tutorials, please let us know.', 'rife-free' ).
		     ' <a href="https://apollo13themes.com/contact/">'.esc_html__( 'Click to contact us.', 'rife-free').'</a>'
		?></p>

	<h3 class="center"><a href="https://support.apollo13.eu/"><?php echo esc_html__( 'Support Forum', 'rife-free' ); ?></a>(Rife Pro only)</h3>
	<p><?php
	...
	<p><?php
		echo esc_html__( 'For Rife Free we provide Basic support via mail. It means we can point you in right direction, explain how something works or fix reported bug.', 'rife-free' ).
		     ' <a href="https://apollo13themes.com/contact/">'.esc_html__( 'Just come and say hi :-)', 'rife-free').'</a>'
		?></p>

	<h2><?php echo esc_html__( 'Changes log:', 'rife-free' ); ?></h2>
	<h3 class="center"><a href="http://www.apollo13.eu/themes_update/apollo13_framework_theme/index.html#change-log"><?php echo esc_html__( 'Visit change log', 'rife-free' ); ?></a></h3>
...
function apollo13framework_apollo13_rife_pro() {
	...
	<p><?php echo esc_html__( 'In Pro version of Rife theme, you get access to much more features, that can help make your page more useful & beautiful. We also add some premium stuff like plugins, that normally you would have to buy separately.', 'rife-free' ); ?></p>
	<p><?php
	...
	<p><?php
		echo '<a href="'.esc_url( $apollo13framework_a13->get_docs_link('support-forum' ) ).'">'.esc_html__( 'You can read more about it in documentation.', 'rife-free' ).'</a>';
		?></p>
...
<div class="wrap apollo13-page <?php echo esc_attr( $current_subpage ); ?> about-wrap">
	...
	<div class="about-text">
		...
		<p class="socials"><a class="on-twitter" href="https://twitter.com/apollo13themes" target="_blank">Apollo13 on Twitter</a></p>
	</div>
