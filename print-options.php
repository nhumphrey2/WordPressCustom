...
function apollo13framework_print_form_controls( $option, &$params, $is_meta = false ) {
	...
	if ( $option['type'] == 'upload' ) {
		...
		<div class="upload-input input-parent"<?php echo strlen($style) ? ' style="'.esc_attr($style).'"' : ''; ?>>
			...
			<div class="input-desc">
				...
				<input id="upload_<?php echo esc_attr($input_prefix . $option['id']); ?>" class="upload-image-button" type="button" value="<?php echo esc_attr($upload_button_text) ?>"<?php
				...
				?> />
				<input id="clear_<?php echo esc_attr($input_prefix . $option['id']); ?>" class="clear-image-button" type="button" value="<?php echo esc_attr__( 'Clear field', 'rife-free' ) ?>" />
	...
	elseif ( $option['type'] == 'color' ) {
		...
		<div class="color-input input-parent"<?php echo strlen($style) ? ' style="'.esc_attr($style).'"' : ''; ?>>
			...
			<div class="input-desc">
				<?php apollo13framework_input_help_tip( __( '<p>Use valid CSS <code>color</code> property values( <code>green, #33FF99, rgb(255,128,0)</code> ), or get your color with color picker tool.</p><p>Use <code>Transparent</code> button to insert transparent value.</p><p>Left empty to use default theme value.</p>', 'rife-free' ) ); ?>
