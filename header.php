...
if ( ! function_exists( 'apollo13framework_header_logo' ) ) {
	...
	function apollo13framework_header_logo() {
		...
		$html = '<a class="'. esc_attr( implode(' ', $normal_logo_classes  ) ) .'" href="' . esc_url( home_url( '/' ) ) . '" title="' . esc_attr( get_bloginfo( 'name', 'display' ) ) . '" rel="home">';
		...
		if ( $img_logo ) {
			if( $svg_logo && strlen( $logo_sizes['normal']['id'] ) && get_post_mime_type( $logo_sizes['normal']['id'] ) === 'image/svg+xml' ){
				...
				$html .= '<img src="' . esc_url( $logo_sources['normal'] ) . '" alt="' . esc_attr( get_bloginfo( 'name', 'display' ) ) . '" width="' . esc_attr( $logo_sizes['normal']['width'] ) . '" height="' . esc_attr( $logo_sizes['normal']['height'] ) . '" />';
			}
		}
		else {
...
if ( ! function_exists( 'apollo13framework_get_header_toolbar' ) ) {
	...
	function apollo13framework_get_header_toolbar( &$icons ) {
		...
		$tools_html =
		        ...
		        ( $header_search ? '<button id="search-button" class="'.esc_attr($header_search_icon).' tool" title="' . esc_attr__( 'Search', 'rife-free' ) . '"><span class="screen-reader-text">' . esc_attr__( 'Search', 'rife-free' ) . '</span></button>' : '' ) .
		        ...
		        ( $menu_overlay ? '<button id="menu-overlay-switch" class="'.esc_attr($menu_overlay_icon).' tool" title="' . esc_attr__( 'Main menu', 'rife-free' ) . '">'.($mo_type === 'animated'? '<i></i>' : '').'<span class="screen-reader-text">' . esc_attr__( 'Main menu', 'rife-free' ) . '</span></button>' : '' ) .
